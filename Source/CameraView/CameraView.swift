...
class CameraView: UIViewController, CLLocationManagerDelegate, CameraManDelegate {
    ...
    func takePicture(_ completion: @escaping (UIImage?) -> Void) {
        ...
        // Capture the photo using cameraMan
        cameraMan.takePhoto(previewLayer, location: locationManager?.latestLocation) { [weak self] image in
            guard let self = self else {
                completion(nil)
                return
            }
            
            // Return the captured image via the completion handler
            completion(image)
        }
    }
